...
class BPGL_XProfile
{
	protected $_context = 'Buddypress Globus', $_field_string_prefix = 'profile field ', $_group_string_prefix = 'profile group ';
	...
	public function bp_init() {
        ...
        if ( isset( $_GET['page'] ) && $_GET['page'] == 'bp-profile-setup' ) {
            wp_enqueue_script( 'bpgl', BPGL_RELPATH . '/js/admin.js', array('jquery'), BPGL_VERSION, true );
        }
    }
    ...
    public function scan_needed_warning() {
        echo '<div class="updated error"><p>'
                . __('Buddypress Globus: some profile fields are not registered for translation','bpgl')
                . '&nbsp;<a class="button edit js-bpgl-register-fields" href="javascript:void(0)" data-bpgl="nonce='
                . wp_create_nonce( 'bpgl-xprofile' )
                . '&action=bpgl_register_fields">'
                . __('Register fields','bpgl') . '</a>'
                . '</p></div>';
    }
    ...
    public function ajax_register() {
        ...
        if ( $this->verify_nonce() ) {
            ...
            $response = __( 'Fields registered', 'bpgl' );
        }
        die( $response );
    }
    
    protected function verify_nonce() {
        if ( !wp_verify_nonce( $_POST['nonce'], 'bpgl-xprofile' ) ) {
            die('0');
        }
        return true;
    }
    ...
    public function sanitize_option_basename( $option, $field_id ) {
        $sanitized_string = bpgl_sanitize_string_name( $option->name, 30 );
