=== Plugin Name ===
Contributors: Aaron Yu
Donate link: http://www.grid.com.tw
Tags: i18n, translation, localization, language, multilingual, WPGlobus, BuddyPress
Requires at least: 3.0
Tested up to: 4.3.1
Stable tag: 1.0.0

BuddyPress Globus allows BuddyPress sites to run fully multilingual using the WPGlobus plugin.

== Description ==

The plugin allows building multilingual BuddyPress sites. It works with single-site or multi-site BuddyPress installations. Both the main site and child blogs can run multilingual.

Guest blogs can choose their language and create multilingual contents. Additionally, each guest can choose the admin language individually .

This plugin requires [WPGlobus](http://www.wpglobus.com/). It uses WPGlobus's language API and hooks to BuddyPress to make it multilingual.

Requirements:

* WPGlobus 1.3.x or higher. 
* Supports BuddyPress versions up to 2.x

= Features =

 * Enables multilingual BuddyPress components
 * Filters all links to maintain right language selection
 * Allows translation of profile fields
 
= Bug issues =
 * Do not use 404 to 301 plugin
